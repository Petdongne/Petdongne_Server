...
class AddressCrawlingSchedulerTest {
    ...
    @BeforeEach
    void setUp() {
        taskExecutor = new TaskExecutor(notificationService);
        addressCrawlingScheduler = new AddressCrawlingScheduler(crawlingService, taskExecutor);
    }
    ...
    void shouldProcessAddressDataWhenNewContentFound(){
        //given
        given(crawlingService.getLatestAddressPost()).willReturn(mockAddressPost);
        ...
        verify(notificationService, times(1)).notify(eq(AddressCrawlingResult.SUCCESS.getMessage()));
    }
    ...
    void shouldSendNotificationWhenThrowingException(){
        //given
        given(crawlingService.getLatestAddressPost()).willThrow(AddressDataCrawlingException.class);
        ...
        //then
        verify(crawlingService, never()).processAddressFile(any());
        verify(notificationService, times(1)).notify(eq(AddressCrawlingResult.FAILURE.getMessage()));
    }
